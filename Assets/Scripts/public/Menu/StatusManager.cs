...
#region FUNC
    ...
    public void UpdateMyStatus()
    {
        ...
        string ATK_PER = ExtraAtkPer > 0? $"공격력 : +{Mathf.Round(ExtraAtkPer * 100)}%\n" : "";
        string ATKSPD_PER = ExtraAtkSpdPer > 0? $"공격속도 : +{Mathf.Round(ExtraAtkSpdPer * 100)}%\n" : "";
        string MOVSPD_PER = ExtraMovSpdPer > 0? $"이동속도 : +{Mathf.Round(ExtraMovSpdPer * 100)}%\n" : "";
        string BAGSTG_PER = ExtraBagStgPer > 0? $"가방용량 : +{Mathf.Round(ExtraBagStgPer * 100)}%\n" : "";
        string INC_TIMER = ExtraIncTimer > DEF_TIMER? $"채굴시간 : +{ExtraIncTimer - DEF_TIMER}초\n" : "";
        string NEXT_SKIP_PER = ExtraNextSkipPer > 0? $"다음층 스킵 : +{Mathf.Round(ExtraNextSkipPer * 100)}%\n" : "";
        ...
        string CHEST_SPAWN_PER = ExtraChestSpawnPer > 0? $"상자 등장확률 : +{Mathf.Round(ExtraChestSpawnPer * 100)}%\n" : "";
        string ORE1_RWD_PER = ExtraOre1RwdPer > 0? $"광석1 획득량 : +{Mathf.Round(ExtraOre1RwdPer * 100)}%\n" : "";
        string ORE2_RWD_PER = ExtraOre2RwdPer > 0? $"광석2 획득량 : +{Mathf.Round(ExtraOre2RwdPer * 100)}%\n" : "";
        string ORE3_RWD_PER = ExtraOre3RwdPer > 0? $"광석3 획득량 : +{Mathf.Round(ExtraOre3RwdPer * 100)}%\n" : "";
        string ORE4_RWD_PER = ExtraOre4RwdPer > 0? $"광석4 획득량 : +{Mathf.Round(ExtraOre4RwdPer * 100)}%\n" : "";
        string ORE5_RWD_PER = ExtraOre5RwdPer > 0? $"광석5 획득량 : +{Mathf.Round(ExtraOre5RwdPer * 100)}%\n" : "";
        string ORE6_RWD_PER = ExtraOre6RwdPer > 0? $"광석6 획득량 : +{Mathf.Round(ExtraOre6RwdPer * 100)}%\n" : "";
        string ORE7_RWD_PER = ExtraOre7RwdPer > 0? $"광석7 획득량 : +{Mathf.Round(ExtraOre7RwdPer * 100)}%\n" : "";
        string ORE8_RWD_PER = ExtraOre8RwdPer > 0? $"광석8 획득량 : +{Mathf.Round(ExtraOre8RwdPer * 100)}%\n" : "";
        string INC_AUTO_ORE_PER = ExtraAutoOrePer > 0? $"자동 광석 수량 : +{Mathf.Round(ExtraAutoOrePer * 100)}%\n" : "";
        string INC_AUTO_CRISTAL_PER = ExtraAutoCristalPer > 0? $"자동 크리스탈 수량 : +{Mathf.Round(ExtraAutoCristalPer * 100)}%\n" : "";
        string DEC_AUTO_PRODUCE_PER = DecAlchemyMaterialPer > 0? $"재료 제작비용 감소 : +{Mathf.Round(DecAlchemyMaterialPer * 100)}%\n" : "";
        string INC_TREASURE_CHEST = ExtraTreasureChest > 0? $"보물상자 획득량 : +{ExtraTreasureChest}\n" : "";
        string INC_AUTO_ORE_BAGSTG_PER = ExtraAutoOreBagStoragePer > 0? $"자동 광석 보관량% : +{Mathf.Round(ExtraAutoOreBagStoragePer * 100)}%\n" : "";
        string INC_AUTO_CRISTAL_BAGSTG_PER = ExtraAutoCristalBagStoragePer > 0? $"자동 크리스탈 보관량% : +{Mathf.Round(ExtraAutoCristalBagStoragePer * 100)}%\n" : "";
